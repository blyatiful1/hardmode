...
fan-out is worth the money, who pays, which model the agents run on, and the patterns
that make runs converge.
...
- A skill or slash command the user invoked tells you to call Workflow (the kit's
  /paranoid-review, /verify-claim, /deep-plan, /bug-hunt qualify — the USER invoking the
  command is the opt-in for that run; a skill you auto-triggered yourself is not).
...
## Model policy (non-negotiable on this machine)
Every `agent()` call carries an explicit `model: 'opus'` (default) or `'sonnet'` (cheap
mechanical stages). Workflow agents NEVER inherit the session model — the driver is the
most expensive model on the box, and a 20-agent fan-out on an inherited driver is a cost
bug, not thoroughness. If a subtask is too demanding for opus, the main loop does it
inline instead of fanning it out. Use `agentType: 'verifier'` (or another kit agent) for
verification stages so verifiers get the read-only toolset — an inline verifier prompt in
a default subagent can edit the code it verifies.
...
`while (budget.total && budget.remaining() > 50_000) { ... }`. The pool is shared across
the main loop and all workflows; stop cleanly before it empties.
...
- Handle null: skipped/dead agents return null. `.filter(Boolean)` and report dropped
  items — never let null read as "clean". A schema that validates when empty needs an
  explicit ran/succeeded flag, or a failed scan reads as a clean result.
...
/paranoid-review (working-diff review, refute-by-default verification), /verify-claim
(3 adversarial refuters + vote on any claim), /deep-plan (judge-panel planning),
/bug-hunt (loop-until-dry whole-repo sweep). Check these before authoring a new script —
