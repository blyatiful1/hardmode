...
- A skill or slash command the user invoked tells you to call Workflow (the kit's
  /paranoid-review, /verify-claim, /deep-plan, /bug-hunt, /big-task, /design-variants,
  /memory-gc, /memory-review all qualify — the USER invoking the command is the opt-in
  for that run; a skill you auto-triggered yourself is not).
...
competing HTML previews, German-market judge when the brief says German),
/memory-gc (corpus-hygiene sweep: dedup/stale/contradiction proposals, absolutize
relative dates), /memory-review (mine the session journal for high-activity sessions
that banked nothing, propose the lessons worth capturing). Check these before
authoring a new script — the pattern you need may already be a command.
